...
#define MODBUS_REGISTERS_DEVICE_FIRSTADDRESS 0
    uint16_t _Registers_Device[MODBUS_REGISTERS_DEVICE_AMOUNT];
    uint8_t _RegistersAttribute_Device[MODBUS_REGISTERS_DEVICE_AMOUNT];
...
#ifdef USE_COIL
    uint8_t _Registers_Gradient[MODBUS_REGISTERS_GRADIENT_AMOUNT];
#else
    uint16_t _Registers_Gradient[MODBUS_REGISTERS_GRADIENT_AMOUNT];
#endif
    uint8_t _RegistersAttribute_Gradient[MODBUS_REGISTERS_GRADIENT_AMOUNT];
#define MODBUS_REGISTERS_DIRECTCOLOR_AMOUNT (LED_TOTAL_COUNT << 1) + 1
#define MODBUS_REGISTERS_DIRECTCOLOR_FIRSTADDRESS 1000
    uint16_t _Registers_DirectColor[MODBUS_REGISTERS_DIRECTCOLOR_AMOUNT];
    uint8_t _RegistersAttribute_DirectColor[MODBUS_REGISTERS_DIRECTCOLOR_AMOUNT];
